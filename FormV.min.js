 ...
 */
var formSetter=function($formname,$data){this.init=function($formname,$data){this.data=$data;this.formname=$formname;for(var $key in $data){var $field=document.forms[$formname].elements[$key];if($field.tagName=='INPUT'){if($field.type=='text'||$field.type==''){$field.value=$data[$key];}else if($field.type=='checkbox'&&$data[$key]=='on'){$field.checked=true;}else if($field.type=='radio'){$field.checked=true;}}
