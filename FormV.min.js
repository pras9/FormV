...
/**
 * formSetter.js 1.0
 * sets form fields from post / given data-set
 * @author pras <pras.svo@gmail.com>
 */
var formSetter=function($formname,$data){this.init=function($formname,$data){this.data=$data;this.formname=$formname;for(var $key in $data){var $field=document.forms[$formname].elements[$key];if($field==undefined)continue;if($field.tagName=='INPUT'){if($field.type=='text'||$field.type==''){$field.value=$data[$key];}else if($field.type=='checkbox'&&$data[$key]=='on'){$field.checked=true;}else if($field.type=='radio'){$field.checked=true;}}
else if($field.tagName=='SELECT'){for(var $i=0;$i<$field.length;$i++){if($field[$i].value==$data[$key])
$field[$i].selected=true;}}
else if($field.tagName=='TEXTAREA'){$field.value=$data[$key];}
else if($field[0]!=undefined){if($field[0].tagName=='INPUT'){for(var $i=0;$i<$field.length;$i++){if($field[$i].value==$data[$key])
$field[$i].checked=true;}}}}}
if($formname==undefined||$data==undefined){return{init:this.init};}
else{this.init($formname,$data);return{};}};